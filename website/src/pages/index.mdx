...
{/_ Mobile-only: just the deploy terminal — the alchemy.run.ts source
reads poorly when stacked above it on narrow screens. _/}
...
{/_ Narrow-screen replacement: a single tabbed card that auto-flips
between the two files with a short caption explaining each step. _/}
