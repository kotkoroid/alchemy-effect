...
So far, Alchemy has been storing state locally in the `.alchemy/`
...
The next deploy creates a state Worker on Cloudflare backed by a
Durable Object with SQLite. From that point on, every deploy reads
and writes state remotely — no shared filesystem needed.
