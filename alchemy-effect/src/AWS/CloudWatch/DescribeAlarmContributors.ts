...
export interface DescribeAlarmContributorsRequest
  extends Omit<cloudwatch.DescribeAlarmContributorsInput, "AlarmName"> {}
...
export const DescribeAlarmContributorsLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const describeAlarmContributors = yield* cloudwatch.describeAlarmContributors;

    return Effect.fn(function* (alarm: AlarmResource) {
      const AlarmName = yield* alarm.alarmName;
      yield* Policy(alarm);

      return Effect.fn(function* (request: DescribeAlarmContributorsRequest = {}) {
